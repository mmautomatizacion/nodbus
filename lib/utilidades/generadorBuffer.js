 ...
 */
var buffer = function(tiposMB, endian, noErrores) {
    if (!(this instanceof buffer)) return new buffer(tiposMB, endian, noErrores);
    this.bytesBuffer = 0;
    this.buff = new Array;
...
buffer.prototype.generar = function() {
    ...
    var e;
    for (e in this.buff) {
        var elemento = this.buff[e];
        var tipo = this.tiposMB[elemento['tipo']];

        var funcion = "write"+tipo['funcion'];
        if (tipo['endian']) {
            funcion += this.endian.toUpperCase();
        }
        console.log(this.noErrores);
        bufferGenerado[funcion](elemento['valor'], offset, this.noErrores);
